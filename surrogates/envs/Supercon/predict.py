...
class Inference:
    def __init__(self, model_path, input_size, hidden_size=128):
        ...
        # Load model
        self.model = TcPredictor(input_size=self.input_size, hidden_size=self.hidden_size)
        state_dict = torch.load(self.model_path, map_location=self.device)
        # Strip "module." prefix if trained with DataParallel
        new_state_dict = {}
        for k, v in state_dict.items():
            if k == "n_averaged": continue # from SWA
...
if __name__ == "__main__":
    ...
    INPUT_SIZE = 509  # Update with the actual feature dimension after training

    # Sample materials for testing
    sample_materials = [
        {"formula": "Y1Ba2Cu2.7Co0.3O6.88", "structure": None},
    ]
    ...
    for material in sample_materials:
        ...
        if predicted_tc is not None:
            print(
                f"Formula: {material['formula']} => Predicted Tc: {predicted_tc:.2f} K"
            )
