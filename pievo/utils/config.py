...
def get_agent_llm_config(agent_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    api_key = os.environ.get(
        api_config.get("api_key_env", ""), api_config.get("api_key", "")
    )

    # Build config_list entry
    config_entry = {"model": api_config.get("model_name", "gpt-4"), "api_key": api_key}
...
def get_tools_for_agent(
    agent_config: Dict[str, Any], available_tools: Dict[str, Any]
) -> List[Any]:
    """
...
def init_results(save_dir, model_cfg_path, model_config, task_cfg_path, task_config):
    os.makedirs(
        "/".join(os.path.join(save_dir, model_cfg_path).split("/")[:-1]), exist_ok=True
    )
    os.makedirs(
        "/".join(os.path.join(save_dir, task_cfg_path).split("/")[:-1]), exist_ok=True
    )
